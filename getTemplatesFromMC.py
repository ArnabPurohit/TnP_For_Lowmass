...
def main(options):
    ...
    for binVar1 in xrange(len(var1s)-1):
        for binVar2 in xrange(len(var2s)-1):
            ...
            cuts = "("+ options.weightVarName+".isfinite()  && (tag_Pho_mva > "+options.tagIDMVAcut+" && tag_Pho_pt > "+options.tagPtcut+"  && "
            ...
            #spError = ROOT.Double(0.)
            #sfError = ROOT.Double(0.)
            #hpassInt = histos[hp].IntegralAndError(1, histos[hp].GetNbinsX(), spError)
            #hfailInt = histos[hf].IntegralAndError(1, histos[hf].GetNbinsX(), sfError)
            if hpassInt !=0:

                StErEff = math.sqrt(hpassInt*(1-hpassInt/(hpassInt+hfailInt)))/(hpassInt+hfailInt)
                print "%.2f %.2f %1.4f %1.6f"%(hpassInt, hfailInt, hpassInt/(hpassInt+hfailInt), StErEff)
...
if __name__ == "__main__":  
    ...
    parser.add_option("", "--tagIDMVAcut", default="-0.6", help="ID MVA cut value for tag")
    parser.add_option("", "--tagPtcut", default="30.0", help="Pt cut value for tag")
